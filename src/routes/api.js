...
router.post('/songs', async (req, res) => {
  ...
  // Auto-detect language from iTunes genre + title/artist text
  const category = detectCategory(title, artist, genre);
  try {
    let energy = null;
    ...
    const result = await pool.query(
      `INSERT INTO songs (spotify_id, title, artist, preview_url, genre, energy, category)
       ...
       RETURNING *`,
      [spotify_id, title, artist, preview_url, genre ?? null, energy, category]
    );
